...
public class ScheduleRepository : IScheduleRepository
{
    ...
    public async Task<IEnumerable<VesselSchedule>> GetAllAsync()
    {
        ...
        const string sql = @"
            SELECT vs.ScheduleId, vs.VesselId, vs.BerthId, vs.ETA, vs.PredictedETA, vs.ETD,
                   ...
                   vs.CargoType, vs.CargoQuantity, vs.CargoUnit,
    ...
    public async Task<IEnumerable<VesselSchedule>> GetActiveByTerminalIdAsync(int terminalId)
    {
        using IDbConnection connection = _connectionFactory.CreateConnection();
        const string sql = @"
            SELECT vs.ScheduleId, vs.VesselId, vs.BerthId, vs.ETA, vs.PredictedETA, vs.ETD,
            ...
            LEFT JOIN BERTHS b ON vs.BerthId = b.BerthId
            WHERE vs.Status NOT IN ('Departed', 'Cancelled')
              AND b.TerminalId = @TerminalId
            ORDER BY vs.ETA";
        return await connection.QueryAsync<VesselSchedule>(sql, new { TerminalId = terminalId });
    }
    ...
    {
        using IDbConnection connection = _connectionFactory.CreateConnection();
        // Clear dependent tables first, then schedules
        const string sql = @"
            ...
            SELECT @@ROWCOUNT;";
        var rowsAffected = await connection.ExecuteScalarAsync<int>(sql);
        return rowsAffected;
    }
}
