...
BEGIN
    ...
    SELECT
        ...
        vs.VesselId,
        v.VesselName,
        vs.EstimatedTimeArrival AS OriginalETA,
        vs.PortId,
        p.PortCode,
    ...
    INNER JOIN dbo.PORTS p ON vs.PortId = p.PortId
    LEFT JOIN LATERAL (
        SELECT TOP 1 Latitude, Longitude, Speed, Heading
        FROM dbo.AIS_DATA
        WHERE VesselId = vs.VesselId
        ORDER BY Timestamp DESC
    ) ais
    WHERE vs.Status IN ('Scheduled', 'In Progress', 'Approaching')
      AND vs.IsActive = 1
      AND v.IsActive = 1
    ORDER BY vs.EstimatedTimeArrival;
END
GO
