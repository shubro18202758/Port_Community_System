...
V3 - Handles IDENTITY columns correctly by letting SQL auto-generate IDs
"""
...
class MundraDataImporterV3:
    ...
    # ==================== IMPORT FUNCTIONS (NO EXPLICIT IDs) ====================

    def import_vessels(self):
        """Import VESSELS - let SQL auto-generate VesselId"""
        print("\n" + "="*60)
    ...
    def import_berths(self):
        """Import BERTHS - let SQL auto-generate BerthId"""
        print("\n" + "="*60)
    ...
    def import_vessel_schedule(self):
        """Import VESSEL_SCHEDULE - let SQL auto-generate ScheduleId"""
        print("\n" + "="*60)
        ...
        # Get vessel ID mapping (name -> auto-generated ID)
        cursor = self.conn.cursor()
        ...
        for _, row in df.iterrows():
            try:
                ...
                if not vessel_id:
                    ...
                    for name, vid in vessel_map.items():
                        if vessel_name in name or name in vessel_name:
                            ...
                            break
                if not vessel_id:
                    vessel_id = 1  # Default to first vessel
                ...
                status_map = {
                    ...
                    'berthed': 'Berthed', 'scheduled': 'Scheduled',
                    'waiting': 'Waiting', 'departed': 'Departed'
                }
    ...
    def import_weather_data(self):
        """Import WEATHER_DATA - let SQL auto-generate WeatherId"""
        print("\n" + "="*60)
    ...
    def import_tidal_data(self):
        """Import TIDAL_DATA - let SQL auto-generate TidalId"""
        print("\n" + "="*60)
    ...
    def import_ais_data(self):
        """Import AIS_DATA - let SQL auto-generate AISId"""
        print("\n" + "="*60)
        ...
        for chunk_num, chunk in enumerate(pd.read_csv(csv_path, chunksize=chunk_size)):
            ...
            for idx, row in chunk.iterrows():
                try:
                    # Use modulo to cycle through vessel IDs
                    ...
                    """, (
                        ...
                        float(row['sog_kn']) if pd.notna(row.get('sog_kn')) else 0.0,
                        float(row['cog_deg']) if pd.notna(row.get('cog_deg')) else 0.0,
                        float(row['cog_deg']) if pd.notna(row.get('cog_deg')) else 0.0,
    ...
    def import_resources(self):
        """Import RESOURCES - let SQL auto-generate ResourceId"""
        print("\n" + "="*60)
...
if __name__ == "__main__":
    ...
    importer = MundraDataImporterV3(CONNECTION_STRING, CSV_BASE_PATH)
    importer.import_all()
